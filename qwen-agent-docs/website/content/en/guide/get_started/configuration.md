...
| `thinking_budget`  | `int` | —        | The maximum number of tokens the model can spend on thinking. Required by DashScope when thinking mode is enabled: it must be a positive integer no greater than the model's maximum thinking length, otherwise the request fails with a `400 InvalidParameter` error. <br>• DashScope: `thinking_budget=12000` <br>• OpenAI-compatible API of DashScope: `'extra_body': {'thinking_budget': 12000}` |
...
llm_cfg = {
    ...
    'generate_cfg': {
        ...
        'thinking_budget': 12000,  # Required by DashScope when thinking mode is enabled
        'use_raw_api': 'True',
        'top_p': 0.8,
    }
}
