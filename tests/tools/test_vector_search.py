...
@pytest.mark.skipif(not os.getenv('DASHSCOPE_API_KEY'), reason='Need DASHSCOPE_API_KEY')
...
def test_vector_search_custom_embedding():
    try:
        from langchain_community.embeddings import FakeEmbeddings
    except ImportError:
        pytest.skip("langchain_community not installed")
        
    # Create fake embeddings with size 1536
    embeddings = FakeEmbeddings(size=1536)
    
    # Pass embeddings in cfg
    tool = VectorSearch(cfg={'embeddings': embeddings})
...
if __name__ == '__main__':
    test_vector_search()
    test_vector_search_custom_embedding()
